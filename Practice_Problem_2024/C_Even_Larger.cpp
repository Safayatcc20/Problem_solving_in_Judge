...
long long min_operations(vector<long long> &a)
{
    ...
    // Strategy 1: decrease odd indices
    for (int i = 0; i < n; i += 2)
    { // 0-based odd indices
        long long left = (i - 1 >= 0 ? a[i - 1] : LLONG_MAX);
        long long right = (i + 1 < n ? a[i + 1] : LLONG_MAX);
        long long max_allowed = min(left, right);
        if (a[i] > max_allowed)
            cost_odd += a[i] - max_allowed;
    }

    // Strategy 2: decrease even indices
    for (int i = 1; i < n; i += 2)
    { // 0-based even indices
        long long left = a[i - 1];
        long long right = (i + 1 < n ? a[i + 1] : LLONG_MAX);
        long long max_allowed = min(left, right);
        if (a[i] > max_allowed)
            cost_even += a[i] - max_allowed;
    }
