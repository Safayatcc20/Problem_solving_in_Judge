...
void dfs(int ind, int station, int present, vector<bool> &vis, vector<vector<int>> &g, vector<int> &cc, int &sum, const vector<vector<int>> &cost)
{
    vis[station] = true;

    for (size_t i = 0; i < g[station].size(); i++)
    {
        int nextStation = g[station][i];
        if (!vis[nextStation])
        {
            ans[station] = max(ans[station], present + cc[nextStation]);
            dfs(nextStation, nextStation, present + cost[station][i], vis, g, cc, sum, cost);
        }
    }
