...
void solve(){
    ...
    n*=2;
    for(int i = 0 ; i <n ;i++){
        if(s[i] == chk[i]) continue;
        else{
            if(s[i] == 'A') pos1.push_back(i);
            if(s[i] == 'B') pos2.push_back(i);
        }
    }
    int ans = 0;
    for(int i = 0 ; i < pos1.size(); i++){
        // cout << pos2[i] << " "<< pos1[i] << sad;
        ans += abs(pos2[i] - pos1[i]);
    }
    pos1.clear();
    pos2.clear();
    chk ="";
    ...
    int ans1 = 0;
    for(int i = 0 ; i < pos1.size(); i++){
        ans1 += abs(pos2[i] - pos1[i]);
    }
    cout << min(ans,ans1) << sad;
}
int32_t main()
{
