...
void solve()
{
    ...
    cin >> n >> k;
    int ninv = (n * (n - 1) / 2) - k;
    vector<vector<bool>> dp(n + 1, vector<bool>(ninv + 1, false));
    vector<vector<int>> prev(n + 1, vector<int>(ninv + 1, -1));
    dp[0][0] = true;

    for (int len = 0; len < n; len++)
    {
        for (int fr = 0; fr <= ninv; fr++)
        {
            if (dp[len][fr] == false)
                continue;
            for (int linc = 1; linc <= n - len; linc++)
            {
                int contri = (linc * (linc - 1)) / 2;
                if (fr + contri > ninv)
                    break;

                if (dp[len + linc][fr + contri] == false)
                {
                    dp[len + linc][fr + contri] = true;
                    prev[len + linc][fr + contri] = linc;
                }
            }
        }
    }
