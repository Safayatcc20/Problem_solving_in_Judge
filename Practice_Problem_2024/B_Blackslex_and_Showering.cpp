...
void solve(){
    ...
    vector<int>suf;
    for(int i = 0 ; i< n -1;i++) {
        pre.push_back(abs(v[i] - v[i+1]));
    }
    for(int i = 1 ; i< pre.size();i++) {
        pre[i] += pre[i-1];
    }
    for(int i = n-1 ; i > 0;i--) {
        suf.push_back(abs(v[i] - v[i-1]));
    
    }
    int sz = pre.size() ;
    ans = min(ans , pre[pre.size() - 1]- pre[0]);
    ans = min(ans , pre[pre.size() - 1]- abs(v[n-1] - v[n-2]));

    for(int i = 1; i < n-1; i++){
        int chk =abs( v[i] - v[i-1]) + abs( v[i] - v[i+1]);
        ans = min(ans , pre[sz-1]- chk + abs(v[i+1]- v[i-1]));
    }
    cout << ans << sad;
