...
int Union(long long a , long long b ){//create a set of group 
...
bool comp(pair<int ,pair<int , pair<int , int>>>&a, pair<int ,pair<int , pair<int , int>>>&b){
...
void solve() {
    ...
    vector<int> ans;
    for (auto &x : v) {
        if (Union(x.first, x.second.first)) {
            ans.push_back(x.second.second.second);
        }
    }
