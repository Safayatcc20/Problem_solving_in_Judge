...
void solve() {
    ...
    multiset<int> mmp;
    for(int i = 1; i <= n ;i++){
        if(mmp.find(mp[v[i] - i]) != mmp.end()) mmp.erase(mmp.find(mp[v[i] - i]));
        mp[v[i]-i]++;
        mmp.insert(mp[v[i]-i]);
        if(i >= k){
            ans[i-k+1] = k - *mmp.rbegin();
            if(mmp.find(mp[v[i-k+1]-(i-k+1)])!= mmp.end()) mmp.erase(mmp.find(mp[v[i-k+1]-(i-k+1)]));
            mp[v[i-k+1]-(i+1-k)]--;
            mmp.insert(mp[v[i-k+1]-(i+1-k)]);
        }
    }
