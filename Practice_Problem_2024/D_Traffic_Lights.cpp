...
void solve(){
    ...
    vector<int>wait(n+1 , 1e18);
    wait[1] = 0;
    int time = 0;
    while(wait[n] == 1e18){
        vector<int>nwait(n+1 , 1e18);
        for(int i = 1; i <= n; i++){
            if(wait[i] == 1e18) continue;
            nwait[i] = min(nwait[i] , wait[i] + 1);
            int j = v[i][time % (deg[i])];
            nwait[j] = min(nwait[j], wait[i]); 
            // cout << i << " "<< j << sad;
        }
        // for(int i = 1  ;i <= n ;i++){
        //     cout << nwait[i] << " ";
