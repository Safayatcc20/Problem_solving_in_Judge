...
void solve(){
    ...
    while(hi < n){
        auto val = hurdle[hi];
        hi++;
        while(pi < m and val.first > powerup[pi].first){
            pq.push(powerup[pi].second);
            pi++;
        }
        cur = val.first-1;
        while(!pq.empty() and cur+jump <= val.second){
            jump += pq.top();
            pq.pop();
            ans++;
        }
        if(cur + jump<= val.second){
            cout << -1 << sad;
            return;
        }
        cur = val.second + 1;
