...
void dfs(int source , vector<pair<int , int>>g[] , vector<vector<bool>>&vis,int val){


    vis[source][val] = true;

    for(auto &x:g[source]){
        int xor_val = val ^ x.second;
        if(vis[x.first][xor_val] == false){
            dfs(x.first, g , vis , xor_val);
        }
    }
}
...
void solve(){
    ...
    while(!q.empty()){
        ...
        for(auto &y: g[x.first]){
            int xor_val = x.second ^ y.second;
            if(vis[y.first][xor_val] == false){
                vis[y.first][xor_val] = true;
                q.push({y.first, xor_val});
            }
        }
    } */
