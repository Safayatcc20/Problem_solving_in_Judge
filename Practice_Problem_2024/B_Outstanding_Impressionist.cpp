...
void solve(){
    ...
    vector<int>freq(2 *n+1), pre(2*n + 1);
    for(int i = 0 ; i <n ;i++){
        if(v[i].first == v[i].second)
        pre[v[i].first]=1,freq[v[i].first]+=1;
    }
    for(int i = 1 ; i <= 2*n ;i++){
        pre[i] = pre[i-1] + pre[i];
    }
    string ans ="";
    for(int i = 0 ; i < n ;i++){
        int l = pre[v[i].first-1];
        int r = pre[v[i].second];
        //cout << r << " "<< l << " "<< v[i].first << " "<< v[i].second << sad;
        if(r - l < v[i].second - v[i].first+1){
            ans+='1';
        }
        else if(v[i].second == v[i].first and freq[v[i].first] <= 1){
            ans+='1';
        }
        else ans+='0';
    }
