...
bool linear_equation(vector<int>&v){
    int n = v.size();
    int a1 = 1 , b1= n , c1 = v[0];
    int a2 = n , b2= 1 , c2 = v[n-1];
    int det = a1* b2 - a2 * b1;
    ...
    if(dx % det != 0 or dy % det != 0) return false;
    ...
    for(int i = 0 ; i < n ;i++){
        if(v[i] != x * ( i + 1) +  y * ( n - i)) return false;
    }
    return true;
}
