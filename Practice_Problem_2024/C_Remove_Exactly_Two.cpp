...
void solve(){
    ...
    multiset<int>degreeval;
    for(int i =1;i<= n;i++){
        degreeval.insert(degree[i]);
    }
    int ans = 0 ;
    for(int i = 1;i <= n; i++){
        auto it = degreeval.find(degree[i]);
        if(it != degreeval.end()){
            degreeval.erase(it);
        }
        vector<int>adj;
        for(auto &x:v[i]){
            if(degreeval.find(degree[x]) != degreeval.end()){
                auto it = degreeval.find(degree[x]);
                if(it != degreeval.end()){
                    degreeval.erase(it);
                }
                adj.push_back(x);
        ...
        for(auto &x:adj){
            degree[x]--;
            degreeval.insert(degree[x]);
        }

        if(!degreeval.empty()){
            ans = max(ans , degree[i] - 1 + *degreeval.rbegin());
           // cout << i << " "<< ans << "  "<<  *degreeval.rbegin()<< sad;
        }

        for(auto &x:adj){
            degreeval.erase(degree[x]);
            degree[x]++;
            degreeval.insert(degree[x]);
        }
        degreeval.insert(degree[i]);
