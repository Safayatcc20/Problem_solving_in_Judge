...
pair<int , int> prod(int l, int r, int k) {
    if (l > r || r - l + 1 < k) return {0,0}; 
    int mid = (l + r) / 2;
    if ((r - l + 1) & 1) { 
        auto x = prod(l , mid-1 , k);
        return {mid + 2 * x.first + x.second * mid, x.second * 2+1};
    } 
    else { 
        auto x = prod(l , mid , k);
        return {2 * x.first+ x.second * mid , x.second * 2};
    }
}
