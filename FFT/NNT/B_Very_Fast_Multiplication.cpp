...
we can use this concept when we need to add two vector element
a = {2 , 4 , 5 } , b = {5 , 6 } how many a + b possible
then create polynomials that a = x^1 + x^3 + x^7 b = x^5 + x^6 + x^2  */
/* vector<int> a(n + 1), b(m + 1); 
   a[1] = a[3] = a[7] = 1;
   a[2] = a[5] = a[6] = 1;
   for(int i = ans.size()-1; i >=0 ;i--) cout << i << " "<< ans[i];
   here i represent the power of x and ans[i] represent how many possible way to get x^i
   just do it
*/
