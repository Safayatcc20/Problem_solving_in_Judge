...
vector<int> multiply(vector<int> &a, vector<int> &b)
{
    ...
    for (int i = 0; i < tt; i++)
        ans[i] = llround(z[i].a);
    for (int i = 0; i < tt; i++)
        ans[i] = min(ans[i] ,1ll); // here is the little change i need to do
        // due to the coefficient can be very large as multiply also i don't need their coeffiecient
        //so i can igonre it just i need to know that is it on or off
    return ans;
}
void solve()
{
