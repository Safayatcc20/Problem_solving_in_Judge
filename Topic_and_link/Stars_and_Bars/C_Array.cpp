...
// C(a , b) = C(a ,a-b)
//stars and bars formula is C(n+ k -1 , n) or C(n + k -1 , n-1) where
// n is stars , k -1 is bars  
...
void solve(){
    int n;
    cin >> n;
    int increasing = ncr(2 * n - 1, n)%mod;
    int decreasing = ncr(2 * n - 1, n)%mod;
    // here 1 1, 2 2 , 3 3 sequence is common for both
    int ans = (increasing + decreasing - n)%mod;
    cout << ans << sad;
