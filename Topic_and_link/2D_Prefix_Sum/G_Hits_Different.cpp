...
vector<vector<int>>pre(2025, vector<int>(2030, 0));
void solve(){
    int n;
    cin >> n;
    int row =  (n*2)*4;
    ...
    row +=1;
    row /=2 ;
    int col = ((1 + row-1) * (row-1)) / 2;
    // cout << n << " "<<row << "  "<< col <<" ";
    col = n - col;
    if(col == 0) col++;
    // cout << n << " "<<row << "  "<< col <<" ";
    cout << pre[row][col] << sad;
}
int32_t main()
{
    ...
    for(int i = 1; i<= 2023; i++){
        for(int j = 1; j<= i ; j++){
            if(j == 1){
                pre[i][j] = pre[i-1][j] + val * val;
                // cout << i << " "<< j << " "<< pre[i][j] << " " <<pre[i-1][j] << " "<< val << sad;
                val++;
            }
            else{
                pre[i][j] = pre[i-1][j-1] + pre[i-1][j]+ val * val;
                if(j > 1){
                    pre[i][j] -=pre[i-2][j-1];
                }
                val++;
