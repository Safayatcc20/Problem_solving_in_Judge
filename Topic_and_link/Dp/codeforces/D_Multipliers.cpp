...
void solve(){
    ...
    int l = v.size();
    vector<int>pre(l+1,0) , suf(l+1,1);
    pre[0] = mp[v[0]]+1;
    for(int i = 1; i < v.size(); i++){
        pre[i] = (pre[i-1] *(mp[v[i]]+1)) % (mod-1);
    }
    suf[l-1] = mp[v[l-1]]+1;
    for(int i = l-2; i >= 0; i--){
        suf[i] = (suf[i+1] *(mp[v[i]]+1)) % (mod-1);
    }
    int ans = 1;
    for(int i = 0 ; i < v.size(); i++){
        ...
        int y = 0;
        if(i == 0) y = suf[i+1];
        else if(i == l-1) y = pre[i-1];
        else y = (pre[i-1]* suf[i+1]) % ( mod - 1);
        x =((( (x * (x+1))))/2)% ( mod -1);
        y = (x * y)%(mod -1 );
        int cal= bin_power(v[i] , y , mod);
        ans = (ans * cal) % mod;
        //cout << v[i] << " "<< x << " "<< y << " "<< cal<< sad;
    }
    cout << ans << sad;
}
int32_t main()
