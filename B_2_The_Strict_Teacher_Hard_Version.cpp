...
void solve(){
    ...
    while(q--){
        ...
        cin >> pos;
        if(pos == 1){
            cout << (abs(v[0] - pos )) << sad;
        }
        else if(pos == n){
            cout << abs(v[m-1] - pos)<< sad;
        }
        else{
            int ind = lower_bound(all(v) , pos)- v.begin();
            //cout << ind <<  " ";
            if(ind == m){
                cout << abs(v[m-1] - n)<< sad;
            }
            else if(pos > v[ind] and ind + 1 < m and  pos < v[ind+1]){
                cout << min(abs(v[ind]-(v[ind] + v[ind+1])/2), abs(v[ind+1]-(v[ind] + v[ind+1])/2))<< sad;
            }
            else if(pos < v[ind] and ind - 1 >= 0 and  pos > v[ind-1]){
